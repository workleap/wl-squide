...
**A declaration that would have contributed something is reported.** Inline `children`, a `$priority`, a `sectionId`, or a `$label` that is a string and is not the registered one are discarded on a re-declaration, so `_validateRegistrations` now names the menu, the section and what each ignored declaration carried. A section declared inside another section's `children` is reported too, since it is dropped from where it was written together with everything declared under it. A re-declaration registered at the root of the menu and carrying none of those is the supported shape and is never reported.
