...
- `registerInstance(key, instance, options?)` accepts a `loadResources` function (`(language) => Promise<ResourceLanguage>`, a namespace to bundle map). When provided, the plugin loads the resources of the current language right away, and the resources of any language the instance doesn't hold before switching to it. The instance must be initialized with `resources: {}` so that i18next initializes synchronously and creates its store. A hybrid instance holding the static resources of one language and lazy-loading the others is supported. Exported types: `LoadResourcesFunction`, `RegisterInstanceOptions`, `i18nextInstanceRegistryEntry`.
- The plugin implements the `Plugin` readiness surface (`isReady()`, `addReadyListener()`, `removeReadyListener()`). It becomes ready once the modules are registered and every registered instance has settled the load of the current language, which keeps `useIsBootstrapping` `true` until the initial resources are loaded. A failed load counts as settled, so the application still renders.
