...
`useIsBootstrapping` now waits for the plugins implementing the new `Plugin` readiness surface (`isReady()`, `registerReadyListener()`, `removeReadyListener()`) to be ready, on the normal path and on the 401 path alike. The `AppRouter` state gained an `arePluginsReady` flag set by a new `plugins-ready` action, mirrored to the `AppRouterStore` and dispatched on the event bus as `PluginsReadyEvent` (`"squide-plugins-ready"`). The Honeycomb bootstrapping span gained a matching `plugins-ready` event.
