...
Lazy loading is an option of `registerInstance(key, instance, { loadResources })` rather than a separate registry or an i18next backend plugin: the plugin owns when a language is loaded and how its bundles are added, and `react-i18next` never suspends. The plugin also owns readiness through the generic surface of `Plugin` (ADR-0009): it is ready once the modules are registered and every instance has settled the load of the current language, a failed load included (fail-open). `registerInstance` throws once the modules are registered, mirroring `registerRoute`.
