...
| Squide sorts a menu's top-level navigation items by `$priority`. A nested item's `$priority` is forwarded to the renderer as the `priority` render prop, not acted on — ordering inside a section is the renderer's decision. | [0026](./0026-priority-is-forwarded-not-enforced.md) |
