...
| The navigation section index and the pending registration index are keyed by the `(menuId, sectionId)` pair, as a `Map` of `Map`s, not by a string joining the two. The index key is no longer public: use `getPendingSections()`. | [0022](./0022-section-index-keyed-by-pair.md) |
