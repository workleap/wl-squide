...
| `$priority` orders navigation items at every depth, not only a menu's top level, and is also forwarded to the renderer as the `priority` render prop. The `sectionId` option made a section's `children` a cross-module composition point, which is what `$priority` exists for. | [0026](./0026-priority-applies-at-every-depth.md) |
