...
|---------|------|----------|
| `@squide/core` | `packages/core` | `Runtime` (abstract base), event bus, logging, plugins (deferred registration scope hook, optional readiness surface `isReady` / `addReadyListener` / `removeReadyListener`), deferred registration scope started listeners |
