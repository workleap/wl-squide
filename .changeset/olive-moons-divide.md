...
`parseSectionIndexKey` follows the new format and now recovers the pair correctly for any ids, including ones containing a `-` or the separator itself, which it could not do before. It stays deprecated. Given an old-format key it returns a single element rather than a pair; keys are produced and consumed within a single bootstrap, so a key that outlives the version that made it is not a case that arises.
