...
**Important:** a deferred registration function runs again on every update (feature flag change or new data). Squide discards everything the previous run registered before replaying, so each run must register the **full set** of items it wants rendered — never the difference since the last run. Squide builds the sections it renders from the registrations rather than storing the objects it was given. Treat a section read back from `getNavigationItems` as read-only, and do not mutate a section after registering it.
