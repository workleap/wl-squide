...
| All framework-owned properties on routes and navigation items use the `$` prefix (`$id`, `$visibility`, `$label`, `$name`). This prevents collision with future React Router properties. Navigation link props are filtered generically on every `$` prefixed key before reaching React Router's `Link`; routes and navigation sections are not filtered. | [0018](./0018-dollar-prefix-convention.md), [0022](./0022-navigation-item-metadata-channel.md) |
| Navigation items separate forwarded props from metadata: `$additionalProps` is spread onto the rendered component, `$meta` is read by the renderer and never spread. | [0022](./0022-navigation-item-metadata-channel.md) |
