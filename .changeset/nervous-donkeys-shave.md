...
`$meta` was added in 9.1.0 ([#665](https://github.com/workleap/wl-squide/pull/665)) as the channel for values the code rendering the menu should *read* rather than forward, as opposed to `$additionalProps`, which is spread onto the rendered component. Only the name changes, the behavior is identical:
...
This should have been a major version, but `$meta` shipped two days ago and no application has adopted it, so it goes out as a minor — the same call that was made in 6.3.0 for `$name` → `$id` ([#206](https://github.com/workleap/wl-squide/pull/206)).
