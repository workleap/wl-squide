...
**Only a declaration that would have contributed something is reported.** For a declaration that lost outright, that means inline `children`, a `$priority`, or a `sectionId` of its own. A bare `{ $id, $label, children: [] }` re-declaration is the supported shape for contributing to a section that no module owns, so reporting it would make the recommended pattern noisy in strict mode and train applications to turn strict mode off. A declaration that kept its place and lost only the identifier is always reported, whatever it carries: two sections of one menu answering to the same `$id` is never intended, and nesting with that `$id` silently reaches only one of them.

`$label` is deliberately not compared. It is a `ReactNode`, rebuilt on every registration, so two identical declarations are never equal by reference and comparing them structurally is not something the registry can do cheaply or correctly. The consequence is stated rather than worked around: a section's `$label` comes from whichever declaration ran first, and the report does not say that two declarations disagreed about it.
...
What is given up is the eager diagnostic for a genuine `$id` collision, where two sections that are meant to be different accidentally share an id. The validation still catches most of them: a section a module means to own carries children or a `$priority`, and both are reported. What becomes silent is two empty, unprioritized, differently-labelled sections colliding, which renders as one section with the wrong label. That is accepted — it is the same input as the supported shared-section pattern, and no rule the registry can apply separates them. The consequence for consumers is a naming discipline rather than an enforcement: a section `$id` is a name in a menu-wide namespace that every module shares, so it needs to be specific enough not to collide by accident. The documentation says that now, in place of recommending uniqueness without saying why.

Evidence: `packages/react-router/src/NavigationItemRegistry.ts` (`#recursivelyAddRegistrations`, `#addSectionIndex`, `#addDuplicateDeclaration`, `#deleteDeferredDuplicateDeclarations`, `DuplicateNavigationSectionDeclarations`), `packages/react-router/src/ReactRouterRuntime.ts` (`#validateNavigationSectionDeclarations`). Migration guide: `docs/updating/migrate-to-firefly-v19.0.md`.
...
- A section's identity-bearing options are first-writer-wins. Two modules declaring `Settings` with different `$label`s produce one section carrying one of the two, non-deterministically. Declare a shared section identically, or give it an owner.
...
- Declarations that lost during a deferred run are discarded by `clearDeferredItems`, static ones are kept. A static misconfiguration is reported for the life of the session, matching how static pending registrations already behave under ADR-0022, and a deferred one is re-derived by each run.
