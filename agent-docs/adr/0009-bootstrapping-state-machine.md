...
Evidence: `packages/firefly/src/AppRouterReducer.ts` (`AppRouterActionType` lists the 13 action types, `useEnhancedReducerDispatch` mirrors each action to the store and the event bus, `usePluginsStatusDispatcher` subscribes to the not-ready plugins). `packages/firefly/src/useIsBootstrapping.ts` combines the boolean conditions, `arePluginsReady` included on both exits. `packages/firefly/src/AppRouterStore.ts` provides the non-React parallel. `packages/firefly/src/useExecuteOnce.ts` handles initial state sync. `packages/core/src/plugins/Plugin.ts` declares the readiness surface. `packages/firefly/tests/eventSequence.test.tsx` ("msw + local modules + readiness-aware plugin") covers the gate.
