...
- **Declare it with an empty `children` array**, and never give it inline children. A `$priority` and a `sectionId` are fine as long as every module passes the same ones — a shared subsection is declared with `sectionId`. What is reported is an option that *differs* from the registered section's, or a `$canRender` it doesn't have, since those are discarded: `_validateRegistrations` throws in development and logs in production.
