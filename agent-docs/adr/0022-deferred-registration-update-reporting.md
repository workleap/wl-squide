...
The honest justification for `minor` is therefore not reachability but usage: these are exported, undocumented internals with zero mentions anywhere in `docs/`, and a consumer exhaustively narrowing `registrationStatus` is not a scenario worth holding a major back for. Anyone doing so gets a compile error rather than silent breakage.
...
- **An update run that rejects is not re-validated**, which is the case that most deserves it. `ModuleManager` completes the scope from a `finally`, so the deferred state is cleared and replayed even when a module's update function throws, but `useUpdateDeferredRegistrations` awaits the update before dispatching the completed event, so a rejection skips the dispatch. Moving the dispatch into a `finally` would fix it and would also reverse a deliberately tested behaviour, so it is left alone here. Applications wire this case through `onError` instead.
- A **static** pending registration tolerated since bootstrap is now re-reported on every update run. `clearDeferredItems` deliberately preserves static pending entries and validation walks the whole index, so a production application carrying a known misconfiguration logs the full report on every flag flip rather than once at startup.
