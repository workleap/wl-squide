...
**The ensure holds wherever the declaration comes from.** A section that is waiting for its own section does not compete for the identifier while it waits, per ADR-0024's first invariant. If it later becomes reachable and finds the identifier taken, it loses exactly as one that found it taken at registration time: it is dropped from the place it was waiting for rather than rendered beside the section that owns the identifier, and it is recorded like any other declaration that lost. Treating it as a genuine collision instead would break the recommended pattern, because a shared section that is itself nested is pending in every module until the section holding it registers, and which module gets there first is not defined. It would also make the outcome depend on that order, rendering one section when the holding section came first and several when it came last.
...
**Only a declaration that would have contributed something is reported.** For a declaration that lost outright, that means inline `children`, a `$canRender` the registered section doesn't have, or a `$priority`, a `sectionId` or a string `$label` that differs from the registered section's. Presence is not enough: an option the registered section already carries is discarded by nobody, and reporting it would make the recommended pattern unusable for a prioritized section or for a shared subsection, both of which every module has to declare with the same options. A declaration written inside another section's `children` is reported as well. It carries no option saying so, but it is dropped from where it was written, together with everything declared under it. A bare `{ $id, $label, children: [] }` re-declaration registered at the root of a menu is the supported shape for contributing to a section that no module owns, so reporting it would make the recommended pattern noisy in strict mode and train applications to turn strict mode off. The same rule decides a declaration that was waiting for its own section, which is what keeps a shared subsection silent.
...
- A section that was waiting for its own section and finds the identifier taken is dropped rather than rendered. Ordering therefore stops being observable: a shared section nested under another one is registered once whether the section holding it arrives before or after the modules declaring it. The declaration that lost keeps its registration record, so it takes the identifier if the section that won does not survive a later clear.
- Declarations that lost outright during a deferred run are discarded by `clearDeferredItems`, static ones are kept. A static misconfiguration is reported for the life of the session, matching how static pending registrations already behave under ADR-0022, and a deferred one is re-derived by each run. A declaration recorded while its section was being indexed is discarded whatever its registration type, since the rebuild records it again for every section it reaches.
...
- The registry keeps state that exists only to be reported, one entry per losing declaration, each holding the caller's item. A correctly configured application produces them too: the recommended pattern has every module declare the shared section, so a section declared by `N` modules records `N - 1` entries, and a module that declares it before each item it registers records one per item. The deferred ones are discarded by the next `clearDeferredItems`, the static ones live as long as the session.
