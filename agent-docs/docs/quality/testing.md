...
Use **agent-browser** (see `.agents/skills/agent-browser/`) to validate sample apps. It is installed as a workspace devDependency and available directly on PATH. A build alone is not sufficient — you must start the dev server and verify pages in a real browser.
