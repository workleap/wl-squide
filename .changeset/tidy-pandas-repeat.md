...
Keys are now joined with a separator that cannot appear in an id. `getPendingSectionIds` still returns these keys, but their content changed. Treat them as opaque, and read the `menuId` and the `sectionId` off the `PendingRegistrationItem` values returned by `getPendingRegistrationsForSection` to identify a section. `parseSectionIndexKey` round-trips correctly again but remains deprecated, the key format is not part of the public contract.
