...
| `$priority` is legal at any depth and is forwarded to the renderer as the `priority` render prop. Squide sorts a menu's top-level items; sorting a section's `children` is decided in the ADR and ships separately. The `sectionId` option made a section's `children` a cross-module composition point, which is what `$priority` exists for. | [0026](./0026-priority-applies-at-every-depth.md) |
