...
**An ignored declaration.** A re-declaration of an already-registered section that carried inline `children`, a `$priority`, or its own `sectionId`. Those options are discarded, so the report names the menu, the section, and what each one would have contributed.
