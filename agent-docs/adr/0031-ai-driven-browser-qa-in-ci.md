...
- **`dogfood.yml` + `dogfood.md`** (same pattern): triggered on a monthly schedule (15th of each month) and on-demand. Invokes the `/dogfood` agent skill for exploratory QA. Max 200 turns. After the session, screenshots and videos from `/tmp/dogfood-output/` are pushed to the `dogfood-evidence` orphan branch (requires `contents: write`). If issues are found, a GitHub issue is created with evidence paths rewritten to `https://raw.githubusercontent.com/workleap/wl-squide/dogfood-evidence/`; stops silently if none.
...
- Both workflows use `agent-browser install --with-deps`. Smoke test starts the dev server (`pnpm dev-endpoints`); dogfood builds and serves via `pnpm serve-endpoints` (a production-like server), adding extra build time (~5–10 min for dogfood including build and QA time).
- Dogfood evidence (screenshots, videos) is stored on the `dogfood-evidence` orphan branch and is publicly accessible via GitHub raw URLs. The branch is force-pushed on each run.
